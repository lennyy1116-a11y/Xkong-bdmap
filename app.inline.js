...
async function savePlace() {
  ...
  if (entryKind === 'institution') Object.assign(data, getTaxonomyFields(data.primaryCategory, data.secondaryCategory));
...
const LEAD_CATEGORY_TAXONOMY = {
  ...
  '心理与身心健康': ['临床心理服务中心','心理辅导／咨询中心','儿童发展／评估中心','成瘾治疗／辅导中心'],
...
const LEAD_SECONDARY_NAME_TO_CODE = Object.fromEntries(Object.entries({
  ...
  CARE:['CARE-02','CARE-03','CARE-04','CARE-05','CARE-07','CARE-08'], MHW:['MHW-02','MHW-03','MHW-04','MHW-06'],
...
function getTaxonomyFields(primary, secondary) {
  ...
  return { primary_l1_code:l1, primary_l1_name:primary || '', primary_l2_code:LEAD_SECONDARY_NAME_TO_CODE[secondary] || '', primary_l2_name:secondary || '', taxonomyVersion:'2026-07-30' };
}
function needsCategoryReview(row) {
  return row.manualReviewStatus === 'required' || row.classificationStatus === 'classified_review_required' || row.classificationStatus === 'REVIEW_REQUIRED';
}
function getLeadCategory(row) {
  const raw = String(row.secondaryCategory || row.type || '').trim();
  const parts = raw.split(/[\/／]/).map(x => x.trim()).filter(Boolean);
  const storedPrimary = String(row.primaryCategory || '').trim();
  let primary = storedPrimary || LEAD_CATEGORY_CODE_TO_NAME[String(row.primary_l1_code || '').trim()] || String(row.primary_l1_name || (parts.length > 1 ? parts[0] : '')).trim();
  let secondary = String(row.secondaryCategory || row.primary_l2_name || (parts.length > 1 ? parts[parts.length - 1] : raw)).trim();
  ...
  const rules = [
    ['美容美体与休闲养生','SPA水疗中心',/\bspa\b|水療|水疗/i],
    ['康复与辅助医疗','物理治疗中心',/物理治療|物理治疗|physiotherap/i],
    ['康复与辅助医疗','综合复康中心',/康復|康复|復康|复康|復健|复健/i],
    ['中医与传统医学','中医骨伤／正骨诊所',/正骨|中醫骨傷|中医骨伤/i],
    ['中医与传统医学','传统疗法／养生馆',/拔罐|刮痧|艾灸|養生館|养生馆/i],
    ['美容美体与休闲养生','按摩／足疗中心',/泰式.*按摩|按摩|足療|足疗|massage/i],
    ['医疗诊疗','牙科／口腔诊所',/牙科|齒科|齿科|口腔|dent(?:al|ist)/i],
    ['医疗诊疗','眼科医疗中心',/眼科|ophthalm/i],
    ['医疗诊疗','视光／验眼中心',/視光|视光|驗眼|验眼|optical|optometr/i],
    ['医疗诊疗','体检中心',/體檢|体检|身體檢查|身体检查|檢查中心|检查中心|health\s*check/i],
    ['医疗诊疗','医疗美容机构',/醫美|医美|整形|medical\s*beauty/i],
    ['医疗诊疗','皮肤科诊所',/皮膚科|皮肤科|derma/i],
    ['医疗诊疗','辅助生殖中心',/生殖|試管|试管|不孕|fertility|ivf/i],
    ['中医与传统医学','中医综合诊所',/中醫|中医|註冊中醫|注册中医|finddoc中醫師/i],
    ['运动健身','瑜伽馆',/瑜伽|yoga/i],
    ['运动健身','健身房／健身中心',/健身|fitness|\bgym\b/i],
    ['心理与身心健康','心理辅导／咨询中心',/心理|輔導|辅导|counsell|psycholog/i],
    ['营养健康与药品零售','注册营养师／营养咨询中心',/營養師|营养师|營養諮詢|营养咨询/i],
    ['美容美体与休闲养生','美甲店',/美甲|nail/i],
    ['美容美体与休闲养生','美睫店',/美睫|睫毛|lash/i],
    ['美容美体与休闲养生','半永久／纹绣中心',/半永久|紋繡|纹绣|microblad/i],
    ['美容美体与休闲养生','生活美容／美体中心',/美容|美體|美体|纖體|纤体|beauty|slimming/i],
    ['健康餐饮','养生茶饮／草本饮品店',/養生茶|养生茶|茶飲|茶饮/i],
    ['健康餐饮','轻食／健康餐厅',/輕食|轻食|沙律|沙拉/i],
    ['健康餐饮','药膳／中式养生餐厅',/藥膳|药膳/i],
    ['医疗诊疗','综合／家庭医生诊所',/診所|诊所|醫務|医务|medical|clinic|ehealth醫護機構/i]
  ];
  const matched = rules.find(rule => rule[2].test(text));
  return matched ? { primary:matched[0], secondary:matched[1] } : { primary:'待复核', secondary:'待复核' };
}
...
function getClinicCategoryLabel(p) {
  ...
  const label = cat.primary === '待复核' ? '待复核' : `${cat.primary}／${cat.secondary}`;
  return needsCategoryReview(p) && label !== '待复核' ? `${label} · 待复核` : label;
}

...
async function savePlace() {
  ...
  data.id = editId || (entryKind === 'point' ? genId() : await allocateCanonicalInstitutionId(data));
...
async function claimLead(id) {
  ...
  const existing = places.find(x => x.id === id || x.id === ['place_', id].join('') || clinicMatchKey(x) === clinicMatchKey(p));
  const now = new Date().toISOString();
  const recordId = existing ? existing.id : canonicalClinicId(p);
  try {
    const saved = await runRevisionedMutation(recordId, existing ? (Number(existing.revision) || 0) : null, current => {
      const data = current ? { ...current } : makeCanonicalOperationalRecord(p, { createdAt:now });
...
async function allocateCanonicalInstitutionId(data) {
  ...
  const counterRef = db.collection('institutionCodeCounters').doc(prefix);
  const baseMax = maxBaseInstitutionSerial(prefix);
  return db.runTransaction(async transaction => {
    const snapshot = await transaction.get(counterRef);
    const counterValue = snapshot.exists ? Number(snapshot.data().value) || 0 : 0;
    const next = Math.max(baseMax, counterValue) + 1;
    transaction.set(counterRef, { value:next, primary_l2_code:data.primary_l2_code, updatedAt:new Date().toISOString() }, { merge:true });
...
function makeImportEditableRecord(match, row) {
  ...
  if (match.isBaseClinic) return makeCanonicalOperationalRecord(match, { createdAt:now });
  return { ...match, id:match.id, isBaseClinic:false, createdAt:now };
}

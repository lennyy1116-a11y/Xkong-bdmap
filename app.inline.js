...
const institutionCodeCountersCollection = db.collection('institutionCodeCounters');
...
function syncCustomTypesFromPlaces() {
  loadCustomTypeOptions(places.filter(isPointEntry).map(p => p && p.type));
}
...
function isInstitutionEntry(p) {
  ...
  if (p.name && (p.address || p.phone || p.contact || p.primary_l1_code)) return true;
  return false;
}
const CANONICAL_INSTITUTION_ID_RE = /^[A-Z]+\d{8}$/;
function normalizeCanonicalInstitutionId(value) {
...
function canonicalClinicId(record) {
  if (!record) return '';
  const canonical = normalizeCanonicalInstitutionId(record.id)
    || normalizeCanonicalInstitutionId(record.sourceBaseId);
  if (canonical) return canonical;
  return isInstitutionEntry(record) ? '' : String(record.id || '').trim();
}
function makeCanonicalOperationalRecord(base, extra = {}) {
...
function init() {
  ...
  placesCollection.onSnapshot(snapshot => {
    ...
    snapshot.forEach(doc => {
      allRecords.push(snapshotRecord(doc));
    });
...
function setEntryKind(kind) {
  ...
  document.getElementById('fName').placeholder = k === 'point' ? '例如：朗豪坊 / 乐富广场活动点' : '例如：仁心堂中医综合诊所';
  ...
  if (k === 'point') {
    ...
    if (!['商场','社区场地','活动地点','议员办事处'].includes(document.getElementById('fType').value)) document.getElementById('fType').value = '商场';
    if (catRow) catRow.style.display = 'none';
    if (secRow) secRow.style.display = 'none';
  } else {
    document.getElementById('fType').value = '商场';
...
function openEditSheet(id) {
  ...
  if (isPointEntry(p)) ensureTypeOption(p.type);
  document.getElementById('fType').value = isPointEntry(p) ? (p.type || '商场') : '商场';
  const catRow = document.getElementById('institutionCategoryRow');
  const secRow = document.getElementById('institutionSecondaryRow');
  if (!isPointEntry(p)) {
    if (catRow) catRow.style.display = '';
    ...
    document.getElementById('fPrimaryCategory').value = category.primary || '';
    handlePrimaryCategoryChange();
    document.getElementById('fSecondaryCategory').value = category.secondary || '';
  } else {
    if (catRow) catRow.style.display = 'none';
    if (secRow) secRow.style.display = 'none';
  }
...
async function savePlace() {
  ...
  if (oldPlace && isPointEntry(oldPlace) !== (entryKind === 'point')) {
    const message = '禁止将已有点位与机构互相转换，请新建对应类型记录';
    feedback('err', message); toast(message); return;
  }
  ...
  const data = {
    ...
    type: entryKind === 'point' ? selectedType : '',
  ...
  if (entryKind === 'institution') {
    const primary = document.getElementById('fPrimaryCategory').value || '';
    const secondary = document.getElementById('fSecondaryCategory').value || '';
  ...
  try {
    ...
    if (err && err.code === 'revision-conflict' && editId) {
      try {
        ...
        if (latest.exists) {
          const latestData = snapshotRecord(latest);
...
const LEAD_CATEGORY_TAXONOMY = {
  '医疗诊疗': ['综合／家庭医生诊所','专科诊所','私家医院','日间医疗中心／日间手术中心','体检中心','医学化验所','医学影像／放射诊断中心','牙科／口腔诊所','眼科医疗中心','视光／验眼中心','皮肤科诊所','医疗美容机构','辅助生殖中心','疫苗接种／旅游健康中心','透析／专病治疗中心','睡眠医学／痛症医疗中心'],
  '中医与传统医学': ['中医综合诊所','中医骨伤／正骨诊所','中医针灸专科地点','中医流动／外展服务','中药房／中药材零售','传统疗法／养生馆'],
  '康复与辅助医疗': ['物理治疗中心','职业治疗中心','言语治疗中心','脊医诊所','听力／听觉服务中心','足病诊疗／足部康复中心','义肢矫形／辅助器具中心','综合复康中心'],
  '护理养老与社区照护': ['上门护理／家居护理','家居照顾及支援队','长者日间护理中心','社区复康日间中心','安老院／护养院／护理院','暂托／喘息服务','认知障碍／记忆照护中心','残疾人士日间训练／支援中心','产后／母婴护理中心'],
  '心理与身心健康': ['精神科诊所','临床心理服务中心','心理辅导／咨询中心','儿童发展／评估中心','社区精神健康／复元中心','成瘾治疗／辅导中心'],
  '营养健康与药品零售': ['注册营养师／营养咨询中心','体重管理／健康管理中心','药房／药店','保健品／营养补充品零售','医疗设备／康复用品店','有机／健康食品零售'],
  '运动健身': ['健身房／健身中心','私人教练工作室','瑜伽馆','普拉提馆','舞蹈／团体健身工作室','拳击／武术训练馆','游泳／水中运动中心','体育会／综合运动中心'],
  '健康餐饮': ['轻食／健康餐厅','养生茶饮／草本饮品店','药膳／中式养生餐厅','素食／纯素餐厅','低糖／低碳／控卡餐饮','健康烘焙／健康甜品','营养餐制作／配送'],
  '美容美体与休闲养生': ['生活美容／美体中心','非医疗皮肤管理中心','SPA水疗中心','按摩／足疗中心','桑拿／浴室／蒸疗中心','美甲店','美睫店','半永久／纹绣中心','头疗／头皮护理中心','产后修复／身体塑形中心']
};
const LEAD_CATEGORY_CODE_TO_NAME = {
  MED:'医疗诊疗', TCM:'中医与传统医学', AHP:'康复与辅助医疗', CARE:'护理养老与社区照护', MHW:'心理与身心健康',
  NUT:'营养健康与药品零售', FIT:'运动健身', FNB:'健康餐饮', BEA:'美容美体与休闲养生'
};
const LEAD_CATEGORY_NAME_TO_CODE = Object.fromEntries(Object.entries(LEAD_CATEGORY_CODE_TO_NAME).map(([code,name]) => [name,code]));
const LEAD_SECONDARY_NAME_TO_CODE = Object.fromEntries(Object.entries({
  MED:['MED-01','MED-02','MED-03','MED-04','MED-05','MED-06','MED-07','MED-08','MED-09','MED-10','MED-11','MED-12','MED-13','MED-14','MED-15','MED-16'],
  TCM:['TCM-01','TCM-02','TCM-03','TCM-04','TCM-05','TCM-06'], AHP:['AHP-01','AHP-02','AHP-03','AHP-04','AHP-05','AHP-06','AHP-07','AHP-08'],
  CARE:['CARE-01','CARE-02','CARE-03','CARE-04','CARE-05','CARE-06','CARE-07','CARE-08','CARE-09'], MHW:['MHW-01','MHW-02','MHW-03','MHW-04','MHW-05','MHW-06'],
  NUT:['NUT-01','NUT-02','NUT-03','NUT-04','NUT-05','NUT-06'], FIT:['FIT-01','FIT-02','FIT-03','FIT-04','FIT-05','FIT-06','FIT-07','FIT-08'], FNB:['FNB-01','FNB-02','FNB-03','FNB-04','FNB-05','FNB-06','FNB-07'],
  BEA:['BEA-01','BEA-02','BEA-03','BEA-04','BEA-05','BEA-06','BEA-07','BEA-08','BEA-09','BEA-10']
}).flatMap(([code,codes]) => (LEAD_CATEGORY_TAXONOMY[LEAD_CATEGORY_CODE_TO_NAME[code]] || []).map((name,i) => [name,codes[i]])));
function getTaxonomyFields(primary, secondary) {
  const l1 = LEAD_CATEGORY_NAME_TO_CODE[primary] || '';
  const l2 = LEAD_SECONDARY_NAME_TO_CODE[secondary] || '';
  return { primary_l1_code:l1, primary_l1_name:primary || '', primary_l2_code:l2, primary_l2_name:secondary || '', taxonomyVersion:'2026-07-30' };
}
function hasValidTaxonomy(row) {
  const primary = String(row.primary_l1_name || '').trim();
  const secondary = String(row.primary_l2_name || '').trim();
  return Boolean(LEAD_CATEGORY_TAXONOMY[primary] && LEAD_CATEGORY_TAXONOMY[primary].includes(secondary) && LEAD_CATEGORY_NAME_TO_CODE[primary] === String(row.primary_l1_code || '').trim() && LEAD_SECONDARY_NAME_TO_CODE[secondary] === String(row.primary_l2_code || '').trim());
}
function needsCategoryReview(row) {
  return !hasValidTaxonomy(row) || row.manualReviewStatus === 'required' || row.classificationStatus === 'classified_review_required' || row.classificationStatus === 'REVIEW_REQUIRED';
}
function getLeadCategory(row) {
  if (!hasValidTaxonomy(row)) return { primary:'待复核', secondary:'待复核' };
  return { primary:String(row.primary_l1_name).trim(), secondary:String(row.primary_l2_name).trim() };
}
...
function enrichLeadRow(row) {
  ...
  return {
    ...
    _leadHaystack: [row.name,row.address,row.contact,row.phone,row.primary_l1_code,row.primary_l2_code,category.primary,category.secondary,getOwnerLabel(row)].join(' ').toLowerCase()
  };
}
function getLeadRows(force) {
  ...
  byKey.forEach(row => {
    const key = canonicalClinicId(row) || row.id;
...
function findOperationalPlaceForRecord(record) {
  if (!record) return null;
  const canonicalId = normalizeCanonicalInstitutionId(record.id) || normalizeCanonicalInstitutionId(record.sourceBaseId);
  if (canonicalId) {
    return places.find(x => x.id === canonicalId)
      || places.find(x => canonicalClinicId(x) === canonicalId)
      || null;
  }
  const rawId = String(record.id || '').trim();
  const direct = places.find(x => x.id === rawId || x.id === 'place_' + rawId || x.sourceBaseId === rawId);
  if (direct) return direct;
  const key = clinicMatchKey(record);
  return key ? places.find(x => !canonicalClinicId(x) && clinicMatchKey(x) === key) || null : null;
}
function canonicalWriteSeed(record, existing, extra = {}) {
  const id = canonicalClinicId(record) || canonicalClinicId(existing);
  if (!id) throw new Error('机构缺少永久唯一代码');
  const legacyId = existing && existing.id !== id ? existing.id : '';
  return { ...(record || {}), ...(existing || {}), ...extra, id, isBaseClinic:false, sourceBaseId:legacyId || (existing && existing.sourceBaseId) || (record && record.sourceBaseId) || undefined };
}
function getEditableLeadPlace(id) {
  const p = findLeadRecord(id);
  if (!p) return null;
  return findOperationalPlaceForRecord(p);
}
...
async function claimLead(id) {
  ...
  const existing = findOperationalPlaceForRecord(p);
...
async function resolveLeadError(id) {
  ...
  const recordId = canonicalClinicId(p) || canonicalClinicId(existing);
  const expectedRevision = existing.id === recordId ? (Number(existing.revision) || 0) : null;
...
function clinicExportRow(p, extra={}) {
  return {
    ...
    primary_l1_name: p.primary_l1_name || '',
    primary_l2_code: p.primary_l2_code || '',
    primary_l2_name: p.primary_l2_name || '',
    contact: p.contact || '',
    phone: p.phone || '',
    lat: p.lat || '',
    lng: p.lng || '',
...
function clinicIdentityKeys(p) {
  ...
  if (p.sourceBaseId) {
    keys.add(String(p.sourceBaseId));
    if (String(p.sourceBaseId).startsWith('place_')) keys.add(String(p.sourceBaseId).slice(6));
  }
...
function getPromotedPlaceForBaseClinic(base) {
  if (!base) return null;
  return findOperationalPlaceForRecord(base);
}
...
async function promoteBaseClinic(id) {
  ...
  if (!p) return;
  const now = new Date().toISOString();
  const data = makeCanonicalOperationalRecord(p, {
    status: '已交流',
...
function isClinicLike(p) { return !isPointEntry(p) && hasValidTaxonomy(p); }
  ...
  reader.onload = async function(e) {
    try {
      ...
      const result = await window.BDMapImportSafety.runSafeImport(ready, {
        persist: async row => {
          const entryKind = row.entryKind === 'point' ? 'point' : 'institution';
          const safeId = entryKind === 'point' ? genId() : await allocateCanonicalInstitutionId(row);
          const record = window.BDMapImportSafety.prepareNewImportedRecord({ ...row, entryKind }, { id:safeId, actor:currentUsername||'匿名' });
...
function compactInstitutionCodePrefix(primaryL2Code) {
  const prefix = String(primaryL2Code || '').toUpperCase().replace(/[^A-Z0-9]/g, '');
  if (!/^[A-Z]+\d{2}$/.test(prefix)) throw new Error('机构二级类目代码无效');
  return prefix;
}
function maxBaseInstitutionSerial(prefix) {
  let max = 0;
  for (const clinic of baseClinics) {
    if (compactInstitutionCodePrefix(clinic.primary_l2_code) !== prefix) continue;
    const id = normalizeCanonicalInstitutionId(clinic.id);
    if (!id || !id.startsWith(prefix)) continue;
    max = Math.max(max, Number(id.slice(prefix.length)) || 0);
  }
  return max;
}
async function allocateCanonicalInstitutionId(data) {
  const prefix = compactInstitutionCodePrefix(data && data.primary_l2_code);
  ...
  return db.runTransaction(async transaction => {
    ...
    for (let attempts = 0; attempts < 1000; attempts++, next++) {
      const candidateId = prefix + String(next).padStart(6,'0');
      const candidateRef = placesCollection.doc(candidateId);
      const candidateSnapshot = await transaction.get(candidateRef);
      if (!candidateSnapshot.exists) { allocatedId = candidateId; break; }
    }
    if (!allocatedId) throw new Error('机构代码分配重试超过上限');
    const allocatedSerial = Number(allocatedId.slice(prefix.length));
    transaction.set(counterRef, { value:allocatedSerial, primary_l2_code:data.primary_l2_code, updatedAt:new Date().toISOString() }, { merge:true });
    return allocatedId;
  });
}
function runRevisionedMutation(id, expectedRevision, mutate, options = {}) {
  ...
  const docRef = placesCollection.doc(canonicalId);
  const auditRef = options.action ? auditLogsCollection.doc() : null;
  return db.runTransaction(async transaction => {
    const snapshot = await transaction.get(docRef);
    const current = snapshot.exists ? snapshotRecord(snapshot) : null;
    ...
    const cleanData = cleanForFirestore({ ...proposed, id:canonicalId });
...
const BATCH_SHOP_HEADERS = ['店铺名称','一级类目','二级类目','地址','区域','联系人','电话','来源','备注','认领人','状态','lat','lng'];
...
async function handleBatchShopFile(event){
  ...
  try{
    ...
    for(let r=2;r<=ws.rowCount;r++){
      ...
      const primaryCategory=getRowAny(obj,['一级类目','一級類目','primary_l1_name']);
      const secondaryCategory=getRowAny(obj,['二级类目','二級類目','primary_l2_name']);
      ...
      const taxonomyFields=getTaxonomyFields(primaryCategory, secondaryCategory);
      const item={ rowNumber:r, name, address, phone, primaryCategory, secondaryCategory, ...taxonomyFields, district, contact, source, note, ownerName, status, lat:parseFloat(latRaw), lng:parseFloat(lngRaw), messages:[], importable:true, coordStatus:'待处理', duplicateMatches:[] };
      if(!hasValidTaxonomy(item)){ item.importable=false; item.messages.push('一级／二级类目不在新类目体系中'); }
...
function renderBatchShopPreview(){
  ...
  tbody.innerHTML=rows.slice(0,220).map(x=>{
    ...
    return `<tr><td>${x.rowNumber}</td><td class="${cls}">${status}</td><td><b>${esc(x.name)}</b><br><span class="muted">${esc(x.primary_l1_name)}／${esc(x.primary_l2_name)} · ${esc(x.source)}</span></td><td>${esc(x.address||'')}</td><td>${esc(x.phone||'')}</td><td>${coord}</td><td>${esc(x.messages.join('；') || '—')}</td></tr>`;
  }).join('');
}
async function downloadBatchShopTemplate(){
  const rows=[{ '店铺名称':'示例中医综合诊所', '一级类目':'中医与传统医学', '二级类目':'中医综合诊所', '地址':'香港九龙观塘开源道XX号XX大厦地下X号铺', '区域':'观塘', '联系人':'陈生', '电话':'91234567', '来源':'销售扫街', '备注':'示例行，可删除', '认领人':currentUsername||'销售A', '状态':'待跟进', 'lat':'', 'lng':'' }];
...
async function exportBatchShopReviewXlsx(){
  ...
  const headers=['row','name','address','primary_l1_code','primary_l1_name','primary_l2_code','primary_l2_name','phone','contact','source','status','lat','lng','coord_status','import_status','importable','messages','failure_message'];
  const seen=new Set();
  const rows=reviewRows.filter(x=>{
    const key=[x.rowNumber,x.name,x.address,x.failureMessage||''].join('|');
    if(seen.has(key)) return false;
    seen.add(key); return true;
  }).map(x=>({ row:x.rowNumber, name:x.name, address:x.address, primary_l1_code:x.primary_l1_code, primary_l1_name:x.primary_l1_name, primary_l2_code:x.primary_l2_code, primary_l2_name:x.primary_l2_name, phone:x.phone, contact:x.contact, source:x.source, status:x.status, lat:x.lat||'', lng:x.lng||'', coord_status:x.coordStatus, import_status:x.importStatus||'', importable:x.importable?'Y':'N', messages:(x.messages||[]).join('；'), failure_message:x.failureMessage||'' }));
...
function makeImportEditableRecord(match, row) {
  const canonicalId = canonicalClinicId(match);
  const existing = findOperationalPlaceForRecord(match);
  const now = new Date().toISOString();
  if (existing && existing.id === canonicalId) return { ...existing };
  if (canonicalId) return canonicalWriteSeed(match, existing, { createdAt:(existing && existing.createdAt) || now });
  if (existing) return { ...existing };
  throw new Error('匹配机构缺少永久唯一代码');
}
